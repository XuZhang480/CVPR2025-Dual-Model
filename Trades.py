...
def main():
    ...
    if usewandb:
        ...
        wandb.init(project="CVPR2025-Appendix",
                   name=watermark)
    ...
    elif args.net == "res18_moe_dense":
        net = MOE_Resnet18_Dense(num_experts=4, num_classes=10, size=size)
    elif args.net == "res18_moe_top_1":
        net = MOE_Resnet18_Top_1(num_experts=4, num_classes=10, size=size)
    elif args.net == "vit_small_moe_dense":
        net = MOE_ViTsmall_Dense(num_experts=4, num_classes=10, size=size, patch=args.patch, dimhead=args.dimhead)
    elif args.net == "vit_small_moe_top_1":
        net = MOE_ViTsmall_Top_1(num_experts=4, num_classes=10, size=size, patch=args.patch, dimhead=args.dimhead)
    ...
    if args.resume:
        ...
        checkpoint = torch.load('./checkpoint/' + args.net + '-Trades.t7'.format(args.patch))
    ...
    scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.00005, step_size_up=500, max_lr=0.001)
    ...
    # pgd_train = PGD_inf(eps=epsilon, step_size=step_size, max_iter=num_step_train, batch_size=BATCH_SIZE)
    # pgd_val = PGD_inf(eps=epsilon, step_size=step_size, max_iter=num_step_val, batch_size=BATCH_SIZE)
    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(loaders['train']):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                loss = criterion(outputs, targets) + beta * (1.0 / BATCH_SIZE) * criterion_kl(F.log_softmax(outputs_adv, dim=1), F.softmax(outputs, dim=1))
            ...
            print(100. * robust_correct / total)
    ...
    def test(epoch):
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/' + args.net + '-Trades.t7')
